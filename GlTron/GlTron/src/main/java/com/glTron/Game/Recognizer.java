...
public class Recognizer {
	...
	private float getx()
	{
		return (xv[0] * (float) Math.sin(xv[1] * mAlpha + xv[2]) - xv[3] * (float) Math.sin(xv[4] * mAlpha + xv[5]));
	}
	
	private float gety()
	{
		return (yv[0] * (float) Math.cos(yv[1] * mAlpha + yv[2] - yv[3] * (float) Math.sin(yv[4] * mAlpha + yv[5])));
	}
	
	private float getdx()
	{
		return (xv[1] * xv[0] * (float)Math.cos(xv[1] * mAlpha + xv[2]) - xv[4] * xv[3] * (float)Math.cos(xv[4] * mAlpha + xv[5]));
	}
	
	private float getdy()
	{
		return -(yv[1] * yv[0] * (float)Math.sin(yv[1] * mAlpha + yv[2]) - yv[4] * yv[3] * (float)Math.sin(yv[4] * mAlpha + yv[5]));
	}
